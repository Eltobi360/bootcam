...
function checkTask(event,currentIndex){
    console.log('La tarea selecionada tiene el indice', currentIndex)
  ...
  tasks.forEach(function(task, index) {
    ...
    list = list + `
      <li class="task__item">
        ...
        />
        <span>${task.title}</span>
        <button onclik="removeTask(even, ${index})">Borrar</button>
      </li>
    `
  })
...
/*function renderTasks(){
    ...
    // TODO : Hacer  que el todo se tache usando solo javascript (10 min)

    let list = '' 
    tasks.forEach(function(task, index) {
       // console.log(task, index)
       // list=list + '' task.tittle
        list = list + '<li class="task__item"> <input type="checkbox" onchange="chekTask(event, ${index})"></input> <span>${task.title}</span> <borrar>Borrar</borrar> </li>'
